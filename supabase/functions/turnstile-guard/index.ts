// turnstile-guard — bot + abuse gate for every sensitive flow. v2 (Day 14).
//
...
//
// VPN policy: Cloudflare is the one who decides whether an IP needs an
// interactive challenge (managed mode). We never block based on IP
// reputation ourselves. We only fail a request when Turnstile EXPLICITLY
// rejects the token; if the Cloudflare API is unreachable (transport/HTTP
// error) we fail OPEN so a network blip can't lock real users out.
//
// Response contract:
//   200 { ok: true }
 ...
 * Returns { ok: true } if verification succeeded; { ok: false, codes, blocked }
 * when Turnstile EXPLICITLY rejected the token; or { ok: true, failOpen }
 * when Cloudflare itself is unreachable/erroring (never punish real users
 * for CF/CDN infrastructure problems — the challenge UI still runs).
 */
async function verifyTurnstile(
  token: string
): Promise<{ ok: boolean; codes?: string[]; failOpen?: boolean }> {
  ...
  // known cause of false `invalid-input-response` rejects behind CDNs/VPNs.
  ...
  try {
    ...
    console.warn("turnstile-guard: Cloudflare unreachable — failing open");
    return { ok: true, codes: ["transport_error"], failOpen: true };
  }
  if (!resp.ok) {
    // CF endpoint down / gateway error — not a token verdict. Fail open.
    console.warn(`turnstile-guard: Cloudflare returned HTTP ${resp.status} — failing open`);
    return { ok: true, codes: [`http_${resp.status}`], failOpen: true };
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!verify.ok) {
      console.warn(
        `turnstile-guard captcha rejected action=${action} codes=${verify.codes?.join(",")}`
      );
      return json(
        req,
