...
function BillCard({
  ...
  return (
    <div className="flex items-center gap-2.5 py-2.5 border-b border-border/50">
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-base shrink-0 ${overdue ? "bg-error/10" : "bg-surface border border-border"}`}>
        {emoji}
      </div>
...
function CategoriesCard() {
  ...
  return (
    ...
    >
      <p className="text-[9px] font-semibold text-secondary tracking-wider uppercase mb-2.5">
        By category
      </p>
      {/* stacked bar */}
...
function TimelineCard() {
  ...
  return (
    ...
    >
      <p className="text-[9px] font-semibold text-secondary tracking-wider uppercase mb-3">
        Payment timeline
      </p>
      <div className="flex items-center">
        {pts.map((p, i) => (
          <div key={p.label} className={`flex items-center ${i < pts.length - 1 ? "flex-1" : "flex-none"}`}>
            <div className={`w-2 h-2 rounded-full shrink-0 ${p.state === "overdue" ? "bg-error ring-2 ring-error/20" : p.state === "upcoming" ? "bg-primary" : "bg-border"}`} />
            {i < pts.length - 1 && (
      ...
      <div className="flex justify-between mt-1.5">
        {pts.map(p => (
          <span key={p.label} className={`text-[9px] ${p.state === "overdue" ? "text-error font-medium" : "text-secondary"}`}>{p.label}</span>
        ))}
      </div>
    </motion.div>
  );
          ...
          <div className="px-3.5 pb-3.5 bg-canvas">
            ...
            <div className="flex gap-1.5 mb-3 overflow-hidden">
              {["All", "Streaming", "Utilities", "Housing"].map((c, i) => (
                <span key={c} className={`text-[9px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${
...
export default function Hero() {
  return (
    <section
      ...
      className="min-h-screen flex items-center pt-[100px] pb-16 relative overflow-hidden bg-canvas"
    >
      {/* Ambient light */}
