			...
			<div className="mt-4">
				{[
					...
					<div key={i} className={`flex items-center gap-2.5 py-1.5 ${i < 4 ? "border-b border-border" : ""}`}>
						<div
							className={`w-4 h-4 rounded shrink-0 flex items-center justify-center border ${r.on ? "bg-accent border-accent" : "bg-transparent border-border"}`}
						>
							{r.on && (
				...
				<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 justify-items-center">
					{steps.map((step, i) => (
						<motion.div
							...
							className="group w-full max-w-[420px] rounded-2xl border border-border bg-surface p-8 transition-all duration-300 hover:-translate-y-1 hover:border-accent/40 hover:shadow-lg"
						>
							<div className="flex flex-col items-center text-center">
								<div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl border border-accent/20 bg-accent/10 font-mono text-sm font-bold text-accent transition-colors duration-300 group-hover:border-accent/40">
									{String(i + 1).padStart(2, "0")}
								</div>
								<div className="flex-1 w-full">
									<h3 className="mb-3 text-lg font-semibold tracking-tight text-primary">{step.title}</h3>
									<p className="mx-auto max-w-[34ch] text-center text-[15px] leading-7 text-secondary">{step.body}</p>
									<div className="mt-6 flex justify-center w-full">{step.detail}</div>
								</div>
							</div>
						</motion.div>
					))}
