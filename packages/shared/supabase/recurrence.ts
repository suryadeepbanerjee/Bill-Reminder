...
export interface RecurrenceApi {
  fetchRecurrencePreview(params: RecurrencePreviewParams): Promise<string[]>;
}

/**
 * Client-bound recurrence preview (RPC `preview_bill_occurrences`).
 * Pure helpers like `buildPreviewParams` live in this module too.
 */
export function createRecurrenceApi(supabase: SupabaseClient): RecurrenceApi {
  return {
    async fetchRecurrencePreview(
