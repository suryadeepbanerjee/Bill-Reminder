...
import type { BillOccurrence, DashboardOccurrence, MarkPaidInput, DeleteTransactionInput } from "./types";
...
export async function markOccurrencePaid(input: MarkPaidInput): Promise<void> {
  // Guarded per occurrence: a rapid double-tap on Mark Paid fires once.
  await guardAsync(`mut:mark-paid:${input.occurrence_id}`, async () => {
    // First: update the occurrence to paid state via RPC
...
export async function deleteOccurrenceTransaction(input: DeleteTransactionInput): Promise<void> {
  // Guarded per occurrence: a rapid double-tap on Delete fires once.
  await guardAsync(`mut:delete-tx:${input.occurrence_id}`, async () => {
