...
serve(async (req: Request) => {
	...
	try {
		...
		const messageMap: Record<string, string> = {
			overdue: `Your bill <strong>${billName || "Your Bill"}</strong> is <strong style="color:#dc2626;">overdue</strong>. Please make the payment at your earliest convenience.`,
			due_today: `Your bill <strong>${billName || "Your Bill"}</strong> is <strong style="color:#ea580c;">due today</strong>. Don't forget to make the payment.`,
			expected_payment: `Your bill <strong>${billName || "Your Bill"}</strong> has a payment expected soon. Please ensure sufficient funds.`,
			generated: `A new occurrence for <strong>${billName || "Your Bill"}</strong> has been generated. Review the details below.`,
			upcoming: `Your bill <strong>${billName || "Your Bill"}</strong> is coming up. Here's a heads-up so you don't miss it.`,
		};
		const message =
			messageMap[status || "upcoming"] ||
			`Your bill "<strong>${billName || "Your Bill"}</strong>" is due on <strong>${dueDate || "soon"}</strong>.`;

		// Render HTML using template module
		const htmlBody = renderNotificationEmail({
			title: subject || "Bill Reminder",
			message,
			billName: billName || "Your Bill",
			amount: amount || "Check details",
			dueDate: dueDate || "Check details",
