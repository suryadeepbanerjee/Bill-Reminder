-- Migration 040: Do not skip past occurrences in the billing engine
-- Users want to see all occurrences, even if they are in the past, so they can manually track them and build history.
...
BEGIN
  ...
  IF v_latest_cycle_start IS NULL THEN
    ...
    IF v_bill.behavior_type IN ('prepaid_validity', 'wallet_balance') THEN
      -- Use anchor_date if present (the user's chosen "Last payment date")
      -- Since it's the LAST payment date, the first unpaid occurrence should be the NEXT one.
      IF v_bill.anchor_date IS NOT NULL THEN
        IF v_bill.repeat_kind = 'monthly' THEN
          v_next_cycle_start := (v_bill.anchor_date + interval '1 month')::date;
        ...
        ELSIF v_bill.repeat_kind = 'yearly' THEN
          v_next_cycle_start := (v_bill.anchor_date + interval '1 year')::date;
        ...
        ELSIF v_bill.repeat_kind = 'every_x_days' THEN
          v_next_cycle_start := (v_bill.anchor_date + (coalesce(v_bill.repeat_interval, 1) || ' days')::interval)::date;
        ELSIF v_bill.repeat_kind = 'every_x_weeks' THEN
          v_next_cycle_start := (v_bill.anchor_date + (coalesce(v_bill.repeat_interval, 1) * 7 || ' days')::interval)::date;
        ELSIF v_bill.repeat_kind = 'every_x_months' THEN
          v_next_cycle_start := (v_bill.anchor_date + (coalesce(v_bill.repeat_interval, 1) || ' months')::interval)::date;
  ...
  -- Remove the LOOP and IF v_due_date < CURRENT_DATE skip block!
  -- We now ALWAYS generate the immediate next cycle, even if it is in the past.
  -- This allows users to build history and manually mark past occurrences as paid.
